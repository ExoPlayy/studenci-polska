...
const nav=(active='')=>`<a class="skip-link" href="#main-content">Przejdź do treści</a><header class="guide-nav"><a class="brand" href="/"><span>SP</span>studenci<b>.</b>polska</a><nav aria-label="Główna nawigacja"><a href="/#miasta">Miasta</a><a href="/#jak-wybrac">Jak wybrać?</a><a href="/poradniki.html"${active==='guides'?' aria-current="page"':''}>Poradniki</a></nav><a class="back" href="/#miasta">Wszystkie miasta →</a></header>`;
const footer=`<footer><a class="brand" href="/"><span>SP</span>studenci<b>.</b>polska</a><p>Miasta, uczelnie, wydarzenia i praktyczne poradniki.</p><small>© 2026 Studenci Polska</small></footer><script src="/analytics.js" defer></script>`;
...
for(const [index,guide] of GUIDES.entries()){
  ...
  const html=`<!doctype html><html lang="pl"><head>${head({title:`${guide.title} | Studenci Polska`,description:guide.description,canonical:url(guide),image:share(guide)})}${schema.map(item=>`<script type="application/ld+json">${JSON.stringify(item)}</script>`).join('')}</head><body>${nav('guides')}<main id="main-content"><div class="guide-breadcrumb"><a href="/poradniki.html">Poradniki</a><span>→</span><span>${escape(guide.title)}</span></div><section class="guides guide-single">${article}</section><section class="guide-related-section"><p class="overline">CZYTAJ DALEJ</p><h2>Powiązane<br><em>poradniki.</em></h2><div class="guide-index">${related.map(item=>`<a href="/poradniki/${item.slug}/"><span>${item.number}</span>${escape(item.title)}</a>`).join('')}</div></section><section class="guide-city-links"><p class="overline">MIASTA AKADEMICKIE</p><h2>Sprawdź grupy<br><em>w swoim mieście.</em></h2><p>Połącz poradnik z lokalnymi informacjami, uczelniami i wydarzeniami.</p><div>${cities.map(city=>`<a href="/${city.slug}/">${escape(city.name)}</a>`).join('')}<a href="/#miasta">Wszystkie miasta →</a></div></section><section class="guide-faq"><p class="overline">PYTANIA I ODPOWIEDZI</p><h2>Najczęstsze<br><em>pytania.</em></h2>${faq.map(([question,answer])=>`<details><summary>${escape(question)}<span>+</span></summary><p>${escape(answer)}</p></details>`).join('')}</section><section class="guide-cta"><h2>Znajdź swoje<br><em>miasto.</em></h2><a href="/#miasta">Przeglądaj miasta →</a></section></main>${footer}</body></html>`;
