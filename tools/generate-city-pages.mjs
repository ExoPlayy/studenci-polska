...
const relatedCitiesFor=city=>{
  const index=data.CITIES.indexOf(city);
  const candidates=[...data.CITIES.filter(item=>item.slug!==city.slug&&item.region===city.region),...[1,3,7,13].map(offset=>data.CITIES[(index+offset)%data.CITIES.length])];
  return [...new Map(candidates.filter(item=>item.slug!==city.slug).map(item=>[item.slug,item])).values()].slice(0,3);
};
const CITY_LOCATIVE={
...
function renderSeo(city,locative,content,groups,faq){
  const guide=content?`<section class="city-seo" aria-labelledby="city-guide-title"><div class="city-seo-head"><div><p class="overline blue">PRZEWODNIK STUDENCKI</p><h2 id="city-guide-title">${escapeHtml(city.name)}<br><em>dla studentów.</em></h2></div><p>Praktyczne informacje na spokojny start. Najważniejsze grupy i wydarzenia pozostają wyżej.</p></div><div class="city-seo-details"><details><summary><span>01</span><strong>Jak wygląda życie studenckie w ${locative}?</strong><i>+</i></summary><div class="seo-detail-body"><p>${escapeHtml(content.life)}</p><p>Decydując się na studia w ${locative}, porównaj uczelnie, lokalizację wydziału, komunikację oraz możliwości spędzania czasu po zajęciach.</p></div></details><details><summary><span>02</span><strong>Atrakcje studenckie i miejsca warte poznania</strong><i>+</i></summary><div class="seo-detail-body"><p>Po zajęciach warto poznać miejsca najlepiej pokazujące charakter miasta:</p><ul>${content.attractions.map(place=>`<li>${escapeHtml(place)}</li>`).join('')}</ul></div></details><details><summary><span>03</span><strong>Mieszkanie i dojazdy na uczelnię</strong><i>+</i></summary><div class="seo-detail-body"><p>Przy szukaniu pokoju w ${locative} sprawdź trasę do konkretnego budynku wydziału w godzinach porannych. Porównaj czynsz, wszystkie opłaty, internet, kaucję oraz komunikację.</p><a href="/poradniki/bezpieczny-wynajem-mieszkania/">Przeczytaj poradnik o bezpiecznym wynajmie →</a></div></details><details><summary><span>04</span><strong>Pierwszy miesiąc studiów w ${locative}</strong><i>+</i></summary><div class="seo-detail-body"><ul><li>Aktywuj pocztę i system swojej uczelni.</li><li>Znajdź bibliotekę, dziekanat i punkt obsługi studenta.</li><li>Dołącz do grupy miasta oraz grupy swojej uczelni.</li><li>Sprawdź wydarzenia integracyjne i koła zainteresowań.</li></ul><a href="/poradniki/pierwsze-tygodnie-na-studiach/">Zobacz checklistę na pierwsze tygodnie →</a></div></details><details><summary><span>05</span><strong>Jak poznać innych studentów?</strong><i>+</i></summary><div class="seo-detail-body"><p>Zacznij od grupy miejskiej, grupy swojej uczelni i osób z zajęć. Śledź wydarzenia integracyjne, koła naukowe, sport akademicki i wolontariat.</p><a href="/poradniki/jak-poznac-ludzi-na-studiach/">Więcej o życiu studenckim po zajęciach →</a></div></details><details><summary><span>06</span><strong>Praktyczny start — najważniejsze poradniki</strong><i>+</i></summary><div class="seo-detail-body guide-links"><a href="/poradniki/akademik-czy-mieszkanie/">Akademik czy mieszkanie?</a><a href="/poradniki/budzet-studenta/">Budżet studenta</a><a href="/poradniki/stypendia-i-pomoc-materialna/">Stypendia</a><a href="/poradniki/jak-przygotowac-sie-do-sesji/">Pierwsza sesja</a><a href="/poradniki/rekrutacja-na-studia-krok-po-kroku/">Rekrutacja</a><a href="/poradniki/jak-wybrac-kierunek-studiow/">Wybór kierunku</a></div></details></div></section>`:'';
...
function renderRelatedCities(city){
  return `<section class="related-cities" aria-labelledby="related-cities-title"><div class="related-cities-head"><p class="overline blue">ODKRYWAJ DALEJ</p><h2 id="related-cities-title">Podobne miasta<br><em>akademickie.</em></h2></div><div class="related-cities-grid">${relatedCitiesFor(city).map(item=>`<a href="${cityUrl(item.slug)}"><img src="${optimizedImage(item.image)}" alt="${escapeHtml(item.name)} — miasto akademickie" loading="lazy" decoding="async"><span>${escapeHtml(item.region)}</span><h3>${escapeHtml(item.name)}</h3><p>${escapeHtml(item.desc)}</p><b>Zobacz miasto →</b></a>`).join('')}</div></section>`;
}
