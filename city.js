...
const seoSection=seoContent?`<section class="city-seo" aria-labelledby="city-guide-title"><div class="city-seo-head"><div><p class="overline blue">PRZEWODNIK STUDENCKI</p><h2 id="city-guide-title">${city.name}<br><em>dla studentów.</em></h2></div><p>Praktyczne informacje na spokojny start. Najważniejsze grupy i wydarzenia pozostają wyżej — tutaj możesz rozwinąć tylko potrzebny temat.</p></div><div class="city-seo-details"><details><summary><span>01</span><strong>Jak wygląda życie studenckie w ${cityLocative}?</strong><i>+</i></summary><div class="seo-detail-body"><p>${seoContent.life}</p><p>Decydując się na studia w ${cityLocative}, warto porównać nie tylko uczelnie, ale też lokalizację wydziału, komunikację i możliwości spędzania czasu po zajęciach.</p></div></details><details><summary><span>02</span><strong>Atrakcje studenckie i miejsca warte poznania</strong><i>+</i></summary><div class="seo-detail-body"><p>Po zajęciach warto zobaczyć miejsca, które najlepiej pokazują charakter miasta:</p><ul>${seoContent.attractions.map(place=>`<li>${place}</li>`).join('')}</ul><p>To dobry punkt wyjścia do poznawania miasta ze znajomymi z roku, grupy uczelnianej albo wydarzeń studenckich.</p></div></details><details><summary><span>03</span><strong>Mieszkanie i dojazdy na uczelnię</strong><i>+</i></summary><div class="seo-detail-body"><p>Przy szukaniu pokoju w ${cityLocative} sprawdź trasę do konkretnego budynku wydziału w godzinach porannych. Tańsza oferta nie zawsze jest korzystniejsza, jeśli wymaga kilku przesiadek albo długich powrotów wieczorem.</p><ul><li>Porównaj czynsz, opłaty, internet i wysokość kaucji.</li><li>Sprawdź przystanki oraz częstotliwość połączeń.</li><li>Obejrzyj okolicę i części wspólne przed podpisaniem umowy.</li><li>Zapisz stan wyposażenia w protokole odbioru.</li></ul><a href="/poradniki/bezpieczny-wynajem-mieszkania/">Przeczytaj poradnik o bezpiecznym wynajmie →</a></div></details><details><summary><span>04</span><strong>Pierwszy miesiąc studiów w ${cityLocative}</strong><i>+</i></summary><div class="seo-detail-body"><ul><li>Aktywuj pocztę i system swojej uczelni.</li><li>Znajdź bibliotekę, dziekanat i punkt obsługi studenta.</li><li>Dołącz do głównej grupy miasta oraz grupy swojej uczelni.</li><li>Sprawdź wydarzenia integracyjne i koła zainteresowań.</li><li>Zapisz terminy składania wniosków o akademik i stypendia.</li></ul><a href="/poradniki/pierwsze-tygodnie-na-studiach/">Zobacz checklistę na pierwsze tygodnie →</a></div></details><details><summary><span>05</span><strong>Jak poznać innych studentów?</strong><i>+</i></summary><div class="seo-detail-body"><p>Najłatwiej zacząć od grupy miejskiej, grupy swojej uczelni i osób z zajęć. W ${cityLocative} warto śledzić również wydarzenia integracyjne, koła naukowe, sport akademicki i wolontariat. Regularna obecność w kilku miejscach zwykle działa lepiej niż próba uczestniczenia we wszystkim.</p><a href="/poradniki/jak-poznac-ludzi-na-studiach/">Więcej o życiu studenckim po zajęciach →</a></div></details><details><summary><span>06</span><strong>Praktyczny start — najważniejsze poradniki</strong><i>+</i></summary><div class="seo-detail-body guide-links"><a href="/poradniki/akademik-czy-mieszkanie/">Akademik czy mieszkanie?</a><a href="/poradniki/budzet-studenta/">Budżet studenta</a><a href="/poradniki/stypendia-i-pomoc-materialna/">Stypendia i pomoc materialna</a><a href="/poradniki/jak-przygotowac-sie-do-sesji/">Pierwsza sesja</a><a href="/poradniki/praca-podczas-studiow/">Praca podczas studiów</a><a href="/poradniki/bezpieczenstwo-studenta/">Bezpieczeństwo i oszustwa</a><a href="/poradniki/rekrutacja-na-studia-krok-po-kroku/">Rekrutacja na studia</a><a href="/poradniki/jak-wybrac-kierunek-studiow/">Jak wybrać kierunek?</a></div></details></div></section>`:'';
...
const relatedCandidates=[
  ...CITIES.filter(item=>item.slug!==city.slug&&item.region===city.region),
  ...[1,3,7,13].map(offset=>CITIES[(CITIES.indexOf(city)+offset)%CITIES.length])
];
const relatedCities=[...new Map(relatedCandidates.filter(item=>item.slug!==city.slug).map(item=>[item.slug,item])).values()].slice(0,3);
const relatedCitiesSection=`<section class="related-cities" aria-labelledby="related-cities-title"><div class="related-cities-head"><p class="overline blue">ODKRYWAJ DALEJ</p><h2 id="related-cities-title">Podobne miasta<br><em>akademickie.</em></h2></div><div class="related-cities-grid">${relatedCities.map(item=>`<a href="${cityUrl(item.slug)}"><img src="${optimizedImage(item.image)}" alt="${item.name} — miasto akademickie" loading="lazy" decoding="async"><span>${item.region}</span><h3>${item.name}</h3><p>${item.desc}</p><b>Zobacz miasto →</b></a>`).join('')}</div></section>`;
